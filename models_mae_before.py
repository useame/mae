...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward_loss(self, imgs, pred):
        ...
        #     loss = nn.TripletMarginLoss(margin=200, swap=False, reduction='mean')
        # else:
    ...
    def forward(self, imgs):
        if self.running_mode == RunningMode.GatePreTrain:
            ...
            dy = self.forward_encoder(imgs)
            m_cfg.mask_mode = MaskMode.Negative
            dx = self.forward_encoder(imgs)
            m_cfg.mask_mode = MaskMode.Anchor
            y = self.forward_encoder(imgs)
            dy = torch.reshape(dy, (self.batch_size, -1))
            dx = torch.reshape(dx, (self.batch_size, -1))
            y = torch.reshape(y, (self.batch_size, -1))
            forward_loss = nn.TripletMarginLoss(margin=200, swap=False, reduction='mean')
            tri_loss = forward_loss(y, dy, dx)
            pos_dist = torch.dist(y, dy)
            neg_dist = torch.dist(y, dx)
            pos_neg = pos_dist.detach() - neg_dist.detach()
            # loss_stats = {
            #     'loss': tri_loss, 'pos_dist': pos_dist, 'neg_dist': neg_dist, 'pos-neg': pos_neg
            # }
            ...
            if self.window == 0:
                m_cfg.mask_mode = MaskMode.Anchor
                outputs = self.forward_encoder(imgs)
                self.window = self.window_period
            else:
                if self.running_mode == RunningMode.FineTuning or self.running_mode == RunningMode.Test:
                    m_cfg.mask_mode = MaskMode.Positive
                outputs = self.forward_encoder(imgs)
