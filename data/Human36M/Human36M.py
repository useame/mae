...
class Human36M:
    def __init__(self, data_split,mode):
        self.data_split = data_split
        self.img_dir = osp.join( '/media/ljc/ljc/imagestar/train')
        self.annot_path = osp.join('/media/ljc/ljc/imagestar/annotations')
        # self.human_bbox_root_dir = osp.join(args.root_dir,'data', 'Human36M', 'bbox_root', 'bbox_root_human36m_output.json')
    ...
    def load_data(self):
        ...
        for aid in db.anns.keys():
            ...
            if self.mode == RunningMode.Test or self.mode == RunningMode.BackboneTest:
                img_frame_num = int(img_path.split('/')[-1][-10:-4])
                if img_frame_num > 50:
                    continue
            if self.mode == RunningMode.FineTuning or self.mode == RunningMode.GatePreTrain:
                img_frame_num = int(img_path.split('/')[-1][-10:-4])
                if img_frame_num > 50:
                    continue
    ...
    def evaluate(self, preds, result_dir):
        ...
        for n in range(sample_num):
            ...
            pred_2d_kpt[:,0] = pred_2d_kpt[:,0] / output_shape[1] * bbox[2] + bbox[0]
            pred_2d_kpt[:,1] = pred_2d_kpt[:,1] / output_shape[0] * bbox[3] + bbox[1]
            pred_2d_kpt[:,2] = (pred_2d_kpt[:,2] / depth_dim * 2 - 1) * (bbox_3d_shape[0]/2) + gt_3d_root[2]
